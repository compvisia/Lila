...
Mat4 Mat4::rotate(float x, float y, float z) {
	...
	result.set(0, 0, cos(Rx) * cos(Ry));
	result.set(0, 1, cos(Rx) * sin(Ry) * sin(Rz) - sin(Rx) * cos(Rz));
	result.set(0, 2, cos(Rx) * sin(Ry) * cos(Rz) + sin(Rx) * sin(Rz));

	result.set(1, 0, sin(Rx) * cos(Ry));
	result.set(1, 1, sin(Rx) * sin(Ry) * sin(Rz) - cos(Rx) * cos(Rz));
	result.set(1, 2, sin(Rx) * sin(Ry) * cos(Rz) + cos(Rx) * sin(Rz));

	result.set(2, 0, -sin(Ry));
	result.set(2, 1, cos(Ry) * sin(Rz));
	result.set(2, 2, cos(Ry) * cos(Rz));
...
Mat4 Mat4::transform(float x, float y, float z, float Rx, float Ry, float Rz, float Sx, float Sy, float Sz) {
	...
	result = multiply(trans, multiply(rot, scalar));
